...
float Heightmap::accheight(int x, int z)
{
	...
	//which of the triangles of the tile is the point on?
...
#if 0
	Log("trinorm = "<<trinorm.x<<","<<trinorm.y<<","<<trinorm.z<<std::endl;
#endif

	MakePlane(&plane.m_normal, &plane.m_d, tri[0], trinorm);

#if 0
	Log("tri0 = "<<tri[0].x<<","<<tri[0].y<<","<<tri[0].z<<std::endl;
	Log("tri1 = "<<tri[1].x<<","<<tri[1].y<<","<<tri[1].z<<std::endl;
	Log("tri2 = "<<tri[2].x<<","<<tri[2].y<<","<<tri[2].z<<std::endl;
#endif

	float y = - ( x*plane.m_normal.x + z*plane.m_normal.z + plane.m_d ) / plane.m_normal.y;

#if 0
	Log("- ( "<<x<<"*"<<plane.m_normal.x<<" + "<<z<<"*"<<plane.m_normal.z<<" + "<<plane.m_d<<" ) / "<<plane.m_normal.y<<" = ");
...
#if 1
				for(std::vector<Vec3f**>::iterator normiter = vertexnormals.begin(); normiter != vertexnormals.end(); normiter++)
				{
					std::vector<Vec3f**>::iterator normiter2 = normiter+1;
