...
void DrawImage(unsigned int tex, float left, float top, float right, float bottom, float texleft, float textop, float texright, float texbottom, float *crop)
{
	...
	glVertexAttribPointer(s->m_slot[SSLOT_POSITION], 2, GL_FLOAT, GL_FALSE, sizeof(float)*0, &v[0]);
	glVertexAttribPointer(s->m_slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*0, &t[0]);
#endif
						...
						float texleft, float textop, float texright, float texbottom)
{
	...
	//glUniform1f(s->m_slot[SSLOT_WIDTH], (float)512);
	//glUniform1f(s->m_slot[SSLOT_HEIGHT], (float)512);
	glUniform1f(s->m_slot[SSLOT_SCREENMAPWIDTH], (float)2048);
	glUniform1f(s->m_slot[SSLOT_SCREENMAPHEIGHT], (float)2048);
	glUniform1f(s->m_slot[SSLOT_BASEDEPTH], basedepth);
	//glUniform1f(s->m_slot[SSLOT_BASEDEPTH], 0);
	glUniform4f(s->m_slot[SSLOT_COLOR], 1.0f, 1.0f, 1.0f, 1.0f);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, renderdepthtex);
	glUniform1i(s->m_slot[SSLOT_SCREENDEPTH], 2);
	...
	glUniform1i(s->m_slot[SSLOT_SPRITEDEPTH], 1);
#endif
...
void DrawDeep(unsigned int difftex, unsigned int depthtex, int basedepth, float left, float top, float right, float bottom, float texleft, float textop, float texright, float texbottom)
{
	...
	glUniform1f(s->m_slot[SSLOT_BASEDEPTH], basedepth - (speddepth + neardepth));

	//glUniform1f(s->m_slot[SSLOT_BASEDEPTH], basedepth - neardepth);
	...
	//glUniform1f(s->m_slot[SSLOT_BASEDEPTH], basedepth - speddepth);
#endif
...
#ifdef PLATFORM_GLES20
	glVertexAttribPointer(s->m_slot[SSLOT_POSITION], 2, GL_FLOAT, GL_FALSE, sizeof(float)*0, v);
	glVertexAttribPointer(s->m_slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*0, t);
#endif
...
void DrawDeepColor(float r, float g, float b, float a, float *v, int nv, GLenum mode)
{
	...
	s = &g_shader[g_curS];
	glUniform4f(s->m_slot[SSLOT_COLOR], r, g, b, a);
...
#ifdef PLATFORM_GLES20
	glVertexAttribPointer(s->m_slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(float)*0, &v[0]);
#endif
