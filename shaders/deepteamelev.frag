...
void main(void)
{
	...
	//vec4 teamel = texture2D(ownermap, gl_TexCoord[0].xy);
	//texel.xyz = texel.xyz * (1.0 - teamel.x) + owncolor.xyz * (teamel.x);
	//texel.xyz = texel.xyz * (1.0 - teamel.w) + owncolor.xyz * (teamel.w);
   ...
   // vec4 scdepthel = texture2D(screendepthmap, screentexcoordratio);
	...
	vec4 elevel = texture2D(elevmap, gl_TexCoord[0].xy);


    byte1 = int(elevel.x * 255.0);
    byte2 = int(elevel.y * 255.0);
    byte3 = int(elevel.z * 255.0);
    byte4 = 0;
    int spelev = byte1 + byte2 * 256 + byte3 * 256 * 256 + int(baseelev);

	float elevratio = (1.0 - min(1,float(spelev)/1000.0f))/1.0;

	//elevratio = 0.0;

	elevratio = elevratio * elevratio;
	//* cos(float(int(g_FragCoord.x*500+500)%1000)/2000.0+gl_FragCoord.y/2.0+1.0) * 10.0;
	//* cos((g_FragCoord.x+gl_FragCoord.y*3.1415)*2.0) * 10.0;
	// * sin(gl_FragCoord.x * 2.0 * 3.1415) * cos(gl_FragCoord.y * 2.0 * 3.1415);

	//////texel = vec4(texel.xyz * (1.0-elevratio) + vec3(0.85,0.63,0.5) * elevratio, texel.w);
	...
	//float depthratio = float(spdepth) / maxrange;

	//texel = vec4(texel.xyz * (1.0-depthratio) + vec3(0.1,0.2,0.1) * depthratio, texel.w);
...
*/
 ...
 /*
   ...
   */ 
    ...
    //texel.x = texel.x * (1.0 - float(spdepth) / maxrange);
    //texel.y = texel.y * (1.0 - float(spdepth) / maxrange);
    //texel.z = texel.z * (1.0 - float(spdepth) / maxrange);
