...
/*
closest pt on line to pt "p"
*/
vec3 ClosePtLn(vec3 p, vec3 a, vec3 b)
{
	vec3 ap = p - a;
	vec3 ab = b - a;

	float ab2 = dot(ab,ab);
	float ap_ab = dot(ab,ap);
	float t = ap_ab / ab2;
...
/*
New method (2):
Give a perspective ray from cam for each screen frag to chase,
and don't have to use matrices here.
*/
...
vec4 texel0;
	...
	//new (2)
	//vec4 chaserayfrom = mvp * 
	vec3 chaserayfrom = camcen;
	vec3 chaserayto = outpos.xyz;
	//vec3 chaserayto = perspc.xyz;
	//vec4 crt = imvp * gl_FragCoord;
	//vec3 chaserayto = (crt.xyz / crt.w);
	//vec3 chaseraydir = chaserayto - chaserayfrom;
	//vec3 chaseraydirn = normalize( chaseraydir );
	//chaserayfrom = chaserayto - chaseraydirn;
	...
	for(step=0; step<steps && offlen >= 20; ++step)
	{
		...
		//new (2)
		//get closest pt from outpos2.xyz to chase ray line
		vec3 closept = ClosePtLn(offpos.xyz, chaserayfrom, chaserayto);
		vec3 offvec = closept - offpos.xyz;
		...
		offlen = length( offpos.xyz - closept );
	}
